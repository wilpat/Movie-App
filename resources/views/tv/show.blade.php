...
@section('content')
    ...
    <div class="tvshow-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row items-center">
            <img src="{{$tvshow['poster_path']}}" alt="{{ $tvshow['name'] }}" class="w-64 md:w-96 item-1" >
            <div class="item-2 movie-details md:ml-24">
                ...
                @if (count($tvshow['videos']['results']) > 0)
                    <div x-data="{isOpen:false}">
                        <div class="mt-12">
                            <button @click="isOpen = true" id="play-button" class="inline-flex items-center bg-orange-500 text-gray-900 rounded font-semibold px-5 py-4 hover:bg-orange-600 transition ease-in-out duration-150" target="_blank">
    ...
    <script>
        ...
        // Handle closing the modal and stopping the video on close of the iframe's modal
            ...
            let vidSrc = "https://www.youtube.com/embed/{{ $tvshow['videos']['results'][0]['key'] }}"
            
			modal.onclick = (e)=>{
		        if(e.target.className == 'fixed top-0 left-0 w-full h-full flex items-center shadow-lg overflow-y-auto'){ 
		          iframe.src = ""
		        }
		    }
